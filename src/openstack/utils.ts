...
// Subnet is allowed in all private ranges, but 4th octet must be 0.
const SUBNET_PRIVATE_CIDR_PATTERN = new RegExp(
  // Class A: 10.0.0.0/8 - 10.255.255.0/32
  '(^(10)(.([2]([0-5][0-5]|[01234][6-9])|[1][0-9][0-9]|[1-9][0-9]|[0-9])){2}.0/(8|9|1[0-9]|2[0-9]|3[0-2])$)' +
    // Class B: 172.16.0.0/12 - 172.31.255.0/32
    '|(^(172).(1[6-9]|2[0-9]|3[0-1])(.(2[0-4][0-9]|25[0-5]|[1][0-9][0-9]|[1-9][0-9]|[0-9])).0/(12|1[3-9]|2[0-9]|3[0-2])$)' +
    // Class C: 192.168.0.0/16 - 192.168.255.0/32
    '|(^(192).(168)(.(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])).0/(16|1[7-9]|2[0-9]|3[0-2])$)',
);
