xdescribe('Public offerings page', () => {
  beforeEach(() => {
    ...
    cy.intercept('GET', '/api/users/me/', {
      ...
      .intercept(
        'GET',
        '/api/marketplace-service-providers/?customer_uuid=895e38d197e748459189f19285119edf',
        {
          fixture: 'marketplace/service_providers.json',
        },
      )
      ...
      .intercept(
        'GET',
        '/api/marketplace-public-offerings/?page=1&page_size=9',
        {
  ...
  it('Assure that pages are visible with incorrect token', () => {
    ...
    cy.get('.detailsCardContainer:contains(Ale)').find('a').click();

    // Wait until page loaded
    cy.get('.serviceProviderHeaderContainer', { timeout: 10000 }).should(
      'contain',
      'Ale',
    );

    // click first offering item
    cy.get('.offeringCard').first().contains('a', 'Details').click();

    // Wait until page loaded
    cy.get('div.publicOfferingDetails', { timeout: 10000 }).should(
      'be.visible',
    );
  });
